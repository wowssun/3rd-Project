...
.infoReque{background: rgb(44, 62, 80); color: white; border-radius: 10px;}
	...
	<section class="module bg-dark-30 parallax-bg restaurant-menu-bg"  style="background-image: url(&quot;/resources/img/title/o.jpg&quot;);">
          <div class="container">
            ...
            <div class="row">
              <div class="col-sm-8 col-sm-offset-2">
                <h2 class="module-title font-alt mb-0">일반 회원 목록</h2>
              </div>
            </div>
          </div>
        </section>
        ...
        <!-- 검색 타입 및 검색 키워드  -->
           <!-- 타입, 아이디 M, 이름 N -->
           <form action="/admin/userList" id="searchFrm">
			  ...
			  <tbody>
			   <c:forEach items="${list }" var="mvo" varStatus="loop">
			    <tr> 
			      ...
			      <td><button class="infoReque" data-mid="${mvo.mid}">조회</button></td>
			    </tr>
			    </c:forEach>
			  </tbody>
			</table>
 			...
 			<div class="text-center">  
 	    		 <%-- 페이징 --%>
				   ...
				   <!-- 현재 페이지 번호 및 출력 게시물 수 전송 폼 -->
					<form action="/admin/userList" id="actionFrm" >
						<input type="hidden" name="pageNum" value="${pageDTO.cri.pageNum }">
						<input type="hidden" name="amount" value="${pageDTO.cri.amount }">
